...
import {
	...
	setupThemecolorSwitch,
	setThemecolorSwitchValue,
	getThemecolorSwitchValue,
	setupQuerySelectorInput,
	setQuerySelectorInputValue,
	getQuerySelectorInputValue,
	setcolorPolicySectionId,
...
fixedPolicies.forEach((fixedPolicySection) => {
	const colorInputWrapper = fixedPolicySection.querySelector(
		".color-input-wrapper",
	);
	const key = colorInputWrapper.dataset.pref;
	setupcolorInput(colorInputWrapper, pref[key], async (color) => {
		pref[key] = color;
		await applySettings();
	});
});
...
function createPolicySection(id, policy) {
	if (policy.headerType === "URL") {
		...
		const templatecolorPolicySection = document.querySelector(
			"#template .policy.color-policy",
		);
		const policySection = templatecolorPolicySection.cloneNode(true);
		setupcolorPolicySection(policySection, id, policy);
		return policySection;
	}
}
...
function setupFlexiblePolicySection(policySection, id, policy) {
		...
		switch (select.value) {
			case "color":
				pref.siteList[id].value =
					getcolorInputValue(colorInputWrapper);
				break;
			case "THEME_color":
				pref.siteList[id].value =
					getThemecolorSwitchValue(themecolorSwitch);
				break;
			case "QUERY_SELECTOR":
	...
	const colorInputWrapper = policySection.querySelector(
		".color-input-wrapper",
	);
	const themecolorSwitch = policySection.querySelector(
		".theme-color-switch",
	);
	...
	let initialUseThemecolor = true;
	let initialQuerySelector = "";
	switch (policy.type) {
		case "color":
			initialcolor = policy.value;
			break;
		case "THEME_color":
			initialUseThemecolor = policy.value;
			break;
		case "QUERY_SELECTOR":
	...
	setupcolorInput(colorInputWrapper, initialcolor, async (newcolor) => {
		pref.siteList[id].value = newcolor;
		await applySettings();
	});
	setupThemecolorSwitch(
		themecolorSwitch,
		initialUseThemecolor,
		async (newUseThemecolor) => {
			pref.siteList[id].value = newUseThemecolor;
			await applySettings();
		},
	);
	setupQuerySelectorInput(
 ...
 * @param {object} policy
 */
async function setupcolorPolicySection(policySection, id, policy) {
	setcolorPolicySectionId(policySection, id);
	const policyHeader = policySection.querySelector(".policy-header");
	const colorInputWrapper = policySection.querySelector(
		".color-input-wrapper",
	);
	const deleteButton = policySection.querySelector("button");
	setupcolorInput(colorInputWrapper, policy.value, async (newcolor) => {
		pref.siteList[id].value = newcolor;
		await applySettings();
	});
	deleteButton.onclick = async () => {
...
function updateFixedPolicySection() {
	fixedPolicies.forEach((fixedPolicySection) => {
		...
		const key = colorInputWrapper.dataset.pref;
		setcolorInputValue(colorInputWrapper, pref[key]);
	});
}
...
async function updateSiteList() {
	for (const id in pref.siteList) {
		...
		if (policy && !policySection) {
			...
			switch (policy.type) {
				case "color":
					setcolorInputValue(colorInputWrapper, policy.value);
					break;
				case "THEME_color":
					setThemecolorSwitchValue(themecolorSwitch, policy.value);
					break;
				case "QUERY_SELECTOR":
					setQuerySelectorInputValue(
						querySelectorInputWrapper,
			...
			policySection.classList.contains("color-policy")
		) {
			const colorInputWrapper = policySection.querySelector(
				".color-input-wrapper",
			);
...
async function updateAllowDarkLightText(nthTry = 0) {
	try {
		...
		if (nthTry > 5) {
			console.error("Could not attain browser color scheme.");
		} else {
			console.warn("Failed to attain browser color scheme.");
			setTimeout(
...
/**
 * Saves the preference to browser storage and triggers color update.
 *
